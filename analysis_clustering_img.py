...
# for i in range(n):
threshold = 70
img = cv2.imread('./Image/sample_spark.jpg', 0)
...
img1 = cv2.imread('./Image/sample2_2.jpg', 0)
...
if cluster_num == cluster_num1 ==3:
    ...
    ax.scatter(B[:, 0], B[:, 1], s=5, c='b', label='coordinate of pixel')
    ax.scatter(C[:, 0], C[:, 1], s=5, c='b', label='coordinate of pixel')
    ax.scatter(center[:, 0], center[:, 1], s=80, c='r',  marker='s', label='center')
    ...
    ay.scatter(E[:, 0], E[:, 1], s=5, c='b', label='coordinate of pixel')
    ay.scatter(F[:, 0], F[:, 1], s=5, c='b', label='coordinate of pixel')
    ay.scatter(center1[:, 0], center1[:, 1], s=80, c='r',  marker='s', label='center')
    plt.legend(bbox_to_anchor=(1, 1), loc='upper right', borderaxespad=0, fontsize=10)
